...
class TorchModel(nn.Module):
    def __init__(self, input_dim, sentence_length, vocab):
        ...
        # self.pool = nn.MaxPool1d(sentence_length)  #第一次用MaxPooling
        self.pool = nn.AvgPool1d(sentence_length)  #第二次用AvgPooling
        self.classify = nn.Linear(input_dim, 3)
        # self.activation = torch.sigmoid     #sigmoid做激活函数
        self.activation = torch.relu   #  第一次调整用relu做激活函数
        self.dropout = nn.Dropout(0.1)
        # self.loss = nn.functional.mse_loss   #loss采用均方差损失
        self.loss = nn.functional.cross_entropy   #第一次调整loss采用交叉熵损失函数
...
def main():
    ...
    optim = torch.optim.Adam(model.parameters(), lr=0.005)   #第一次调整使用Adam 建立优化器
    # optim = torch.optim.SGD(model.parameters(), lr=0.005)   #第二次调整使用SGD 建立优化器
